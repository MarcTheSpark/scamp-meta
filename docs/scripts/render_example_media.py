...
unless --force.
    ...
    python3 docs/scripts/render_example_media.py            # render all
    python3 docs/scripts/render_example_media.py hello      # only paths containing "hello"
    python3 docs/scripts/render_example_media.py --force --timeout 300
...
DOCS_DIR = Path(__file__).resolve().parents[1]         # the workspace docs/ dir
sys.path.insert(0, str(DOCS_DIR))
from regenerate_index import PACKAGES                   # noqa: E402  {package: <pkg>/examples dir}
MEDIA_DIR = DOCS_DIR / "_static" / "media"
...
def examples_to_render(name_filter):
    """(examples_root, path) for every example script across the packages, skipping the
    JunkDrawer and caches. rel = path.relative_to(root) is the topic path (the media key)."""
...
def render(path, examples_root, timeout, fade, scale, plot_scale, skip_audio=False, skip_score=False):
...
def main():
    ...
    counts = {}
    for examples_root, path in examples_to_render(args.filter):
        rel = path.relative_to(examples_root)
        mp3 = MEDIA_DIR / rel.with_suffix(".mp3")
        ...
        if skip_audio and skip_score:       # nothing to render here -- both made by hand
            ...
            status = render(path, examples_root, override.get("timeout", args.timeout),
