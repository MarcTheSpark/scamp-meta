...
- **Hello World** — `scamp:Tutorial/01_hello_world.py`
  <br>Plays a C major arpeggio. — *API:* `Session`, `new_part`, `play_note`
- **Tempo Change** — `scamp:Tutorial/02_tempo_change.py`
  <br>Same as Hello World example, but repeatedly, with changing tempi. — *API:* `Moment`, `Session`, `new_part`, `play_note`, `set_tempo_target`
- **Blocking False** — `scamp:Tutorial/03_blocking_false.py`
  <br>By default `play_note` blocks execution until the note is done playing. This is very useful for playing a melody, since you simply call `play_note` in sequence for every note of the melody. However, one simple way of allowing notes to overlap — if desired — is to pass the keyword argument `blocking=False`. This is essentially a single-note fork: playback continues on to the next line without waiting for the note to finish. — *API:* `Session`, `new_part`, `play_note`, `wait`, `wait_for_children_to_finish`
- **Play Chord** — `scamp:Tutorial/04_play_chord.py`
  <br>Demonstrates `ScampInstrument.play_chord` by playing a famous progression, and then playing it a few more times randomly up and down the keyboard. — *API:* `Moment`, `ScampInstrument`, `Session`, `new_part`, `play_chord`, `set_tempo_target`
- **Random Durations and Rests** — `scamp:Tutorial/05_random_durations_and_rests.py`
...
- **Generating Notation** — `scamp:Tutorial/06_notation.py`
  <br>Plays a simple C Major arpeggio, and generates notation for it. — *API:* `Performance`, `Score`, `Session`, `new_part`, `play_note`, `print_lilypond`, `start_transcribing`, `to_score`
- **Time Signatures** — `scamp:Tutorial/07_time_signature.py`
  <br>Plays a simple C Major arpeggio, and generates notation for it in three different ways: - with a 3/8 time signature - with a 3/8 time signature for the first measure followed by 2/4 - with alternating 3/8 and 2/4 time signatures - with a list of bar lengths determining time signatures — *API:* `Score`, `Session`, `new_part`, `play_note`, `start_transcribing`, `to_score`
- **Quantization of Random Floating-Point Durations** — `scamp:Tutorial/08_random_quantized.py`
  <br>Similar to the "Random Durations and Rests" example, except that now we generate notation. Since the lengths are floating point, quantization occurs in the call to "to_score" — *API:* `Session`, `new_part`, `play_note`, `start_transcribing`, `to_score`, `wait`
- **Simplified Quantization** — `scamp:Tutorial/09_simplified_quantization.py`
  <br>Same as last example except that we see two different levers for simplifying the notation: `max_divisor` (which defaults to 8) sets a hard cap on how finely we can divide the beat. `simplicity_preference` (which defaults to 2), rather than setting a hard cap, amplifies the error calculation for complicated divisors, leading to simpler beat divisions unless the complicated division is a very good match. — *API:* `Session`, `new_part`, `play_note`, `start_transcribing`, `to_score`, `wait`
- **Multi-Part Music** — `scamp:Tutorial/10_multi_part.py`
  <br>Plays two coordinated but independent parallel parts, one for oboe and one for bassoon. — *API:* `Session`, `fork`, `new_part`, `play_note`, `start_transcribing`, `to_score`, `wait_for_children_to_finish`
- **Nested Tempi** — `scamp:Tutorial/11_nested_tempi`
  <br>Music running with nested tempi: a trombone plays a cyclic quarter-note pattern on the session's accelerating pulse, while a trumpet, forked onto a child clock, plays a similar eighth-note pattern. After a few beats, while the overall session is still accelerating, the trumpet clock starts decelerating to half speed. The music is then notated two ways: nested_tempi.py transcribes from the session's point of view so that the trombone plays straight quarter notes, while the trumpet shows a written-out deceleration from eighth to quarter notes; nested_tempi_child_clock.py records from the trumpet clock's point of view, yielding plain eighth notes in the trumpet and more complex rhythmic notation in the trombone. — *API:* `Clock`, `MetricPhaseTarget`, `Moment`, `Session`, `fork`, `new_part`, `play_note`, `set_rate_target`, `set_tempo_target`, `start_transcribing`, `to_score`
- **Envelopes (basic)** — `scamp:Tutorial/12_envelopes_basic.py`
  <br>Create and plot two simple envelopes, one with evenly-spaced linear segments, and one with uneven, curved segments. — *API:* `Envelope`, `from_levels`, `show_plot`
- **Envelopes (advanced)** — `scamp:Tutorial/13_envelopes_advanced.py`
  <br>A more comprehensive list of ways to construct and modify Envelopes. — *API:* `Envelope`, `adsr`, `append_envelope`, `ar`, `asr`, `from_function`, `from_levels`, `from_levels_and_durations`, `from_list`, `from_points`, `prepend_envelope`, `release`, `...`
- **Glissando from Envelope** — `scamp:Tutorial/14_gliss_from_envelope.py`
...
- **Envelope Shorthand** — `scamp:Tutorial/15_envelope_shorthand.py`
...
- **Pitch Spelling** — `scamp:Tutorial/21_pitch_spelling.py`
  <br>Demonstrates the ability to define note pitch spelling using the optional properties argument to "play_note". This can be done by explicitly setting it, or by defining the key in which it resides. — *API:* `Session`, `new_part`, `play_chord`, `play_note`, `start_transcribing`, `to_score`
- **Staff Text** — `scamp:Tutorial/22_staff_text.py`
  <br>Demonstrates various ways of adding text annotations to notes that are played. Text is one of the various notational details that can be passed to the fourth, optional "properties" argument of play_note. — *API:* `Session`, `Staff`, `StaffText`, `new_part`, `play_chord`, `play_note`, `start_transcribing`, `to_score`
- **Special Notations** — `scamp:Tutorial/23_special_notations.py`
...
- **Spanners** — `scamp:Tutorial/29_spanners.py`
  <br>Demonstration of starting and stopping various spanners, such as slurs, hairpins, trills, brackets, and pedal lines. Note that there is some finickiness with both lilypond and MusicXML output. LilyPond does not allow multiple of the same spanner at the same time (at least in the same voice), whereas MusicXML does. If exporting to MusicXML, you can keep the spanners straight by providing a label. On the other hand, implementations of MusicXML in many notation programs mangle spanner input. So there's that. — *API:* `Session`, `StartSlur`, `StopSlur`, `fork`, `new_part`, `play_chord`, `play_note`, `start_transcribing`, `time`, `to_score`

## Time & clocks

### Forking & simultaneity

- Multi-Part Music — *(home: `scamp:Tutorial/10_multi_part.py`)*
- Blocking False — *(home: `scamp:Tutorial/03_blocking_false.py`)*
- Play Chord — *(home: `scamp:Tutorial/04_play_chord.py`)*
- **Chords and Noteheads** — `scamp:Time & clocks/Forking & simultaneity/chords_example.py`
  <br>Chords with per-note noteheads, and a start_chord handle whose pitches change over time. Under the hood, `play_chord` plays all but one of its notes with `blocking=False` and the last one blocking, so the notes sound together. — *API:* `Session`, `change_pitch`, `end`, `new_part`, `play_chord`, `start_chord`, `start_transcribing`, `to_score`, `wait` — *also under:* Time & clocks › Forking & simultaneity, Playback › Note handles
- Voice Leading / spelling policy *(scamp_extensions)* — *(home: `scamp_extensions:Pitch/Pitch spelling/voice_leading.py`)*
- **Guitar Arpeggios** — `scamp:Time & clocks/Forking & simultaneity/guitar_arpeggios.py`
  <br>Fingerpicking-style guitar arpeggios built from overlapping held notes. — *API:* `Session`, `new_part`, `play_note`, `wait`
- **Octave Lines** — `scamp:Time & clocks/Forking & simultaneity/octave_lines.py`
...
- Tempo Change — *(home: `scamp:Tutorial/02_tempo_change.py`)*
- Piano Phase — *(home: `scamp:Composition & form/Larger-scale form/reich_piano_phase.py`)*
- Evolving Form *(scamp_extensions)* — *(home: `scamp_extensions:Composition & form/Larger-scale form/evolving_form`)*

### Advanced

- Nested Tempi — *(home: `scamp:Tutorial/11_nested_tempi`)*
- **Metric Phase** — `scamp:Time & clocks/Advanced/metric_phase.py`
...
- **Scales** *(scamp_extensions)* — `scamp_extensions:Pitch/Scales & microtonality/scale_example`
...
- Microtonal Playback and Notation — *(home: `scamp:Tutorial/18_microtonal.py`)*

### Pitch spelling

- Pitch Spelling — *(home: `scamp:Tutorial/21_pitch_spelling.py`)*
- **Voice Leading / spelling policy** *(scamp_extensions)* — `scamp_extensions:Pitch/Pitch spelling/voice_leading.py`
  <br>Rule-based four-part voice leading over a scale, spelled in E major via the `default_spelling_policy` argument. — *API:* `Session`, `Voice`, `new_part`, `play_chord`, `play_note`, `start_transcribing`, `to_score` — *also under:* Time & clocks › Forking & simultaneity, Pitch › Pitch spelling

### Glissando

- Glissando from Envelope — *(home: `scamp:Tutorial/14_gliss_from_envelope.py`)*
- Envelope Shorthand — *(home: `scamp:Tutorial/15_envelope_shorthand.py`)*
...
- Envelopes (basic) — *(home: `scamp:Tutorial/12_envelopes_basic.py`)*
- Envelopes (advanced) — *(home: `scamp:Tutorial/13_envelopes_advanced.py`)*

### As note parameters

- Glissando from Envelope — *(home: `scamp:Tutorial/14_gliss_from_envelope.py`)*
- Envelope Shorthand — *(home: `scamp:Tutorial/15_envelope_shorthand.py`)*
- Volume Envelope — *(home: `scamp:Tutorial/17_volume_envelope.py`)*

### As compositional parameters

- Evolving Form *(scamp_extensions)* — *(home: `scamp_extensions:Composition & form/Larger-scale form/evolving_form`)*
- TimeVaryingParameter Extension *(scamp_extensions)* — *(home: `scamp_extensions:Composition & form/Larger-scale form/time_varying_parameter_example.py`)*
...
- Generating Notation — *(home: `scamp:Tutorial/06_notation.py`)*
- Time Signatures — *(home: `scamp:Tutorial/07_time_signature.py`)*
- **Export to MIDI File** — `scamp:Notation & engraving/Basics/record_and_export_midi.py`
  <br>Records glissandi with microtonal pitches and playback params (fast-forwarded), then exports the performance as a MIDI file. — *API:* `Envelope`, `Moment`, `Session`, `export_to_midi_file`, `fast_forward`, `fork`, `kill`, `new_part`, `play_note`, `set_rate_target`, `start_transcribing` — *also under:* Notation & engraving › Basics, Playback › MIDI CC & arbitrary parameters

### Quantization

- Quantization of Random Floating-Point Durations — *(home: `scamp:Tutorial/08_random_quantized.py`)*
- Simplified Quantization — *(home: `scamp:Tutorial/09_simplified_quantization.py`)*
- **Quantization Comparison** — `scamp:Notation & engraving/Quantization/quantization.py`
...
- Note Properties — *(home: `scamp:Tutorial/20_note_properties.py`)*
- Staff Text — *(home: `scamp:Tutorial/22_staff_text.py`)*
- Special Notations — *(home: `scamp:Tutorial/23_special_notations.py`)*
- Spanners — *(home: `scamp:Tutorial/29_spanners.py`)*
- **Multiple Voices** — `scamp:Notation & engraving/Special notations/voices.py`
...
- **Multiple Voices with Octave Lines** — `scamp:Notation & engraving/Special notations/voices_with_octave_lines.py`
...
- Microtonal Playback and Notation — *(home: `scamp:Tutorial/18_microtonal.py`)*
- **Performance Post-Processing** — `scamp:Notation & engraving/Special notations/post_processing.py`
...
- **Key Signature via pymusicxml** — `scamp:Notation & engraving/Abjad & pymusicxml tweaks/key_sig.py`
...
- **Double Score** — `scamp:Notation & engraving/Abjad & pymusicxml tweaks/double_score.py`
...
- Playback Implementations — *(home: `scamp:Tutorial/19_playback_implementations.py`)*
- OSC to SuperCollider — *(home: `scamp:Tutorial/24_osc_to_supercollider`)*
- **OSC Instrument Playback** — `scamp:Playback/Playback implementations/osc_playback`
...
- **MultiPresetInstrument** *(scamp_extensions)* — `scamp_extensions:Playback/Playback implementations/multipreset_example.py`
...
- Start and End Note — *(home: `scamp:Tutorial/16_start_and_end_note.py`)*
- Chords and Noteheads — *(home: `scamp:Time & clocks/Forking & simultaneity/chords_example.py`)*
...
- Export to MIDI File — *(home: `scamp:Notation & engraving/Basics/record_and_export_midi.py`)*
- **Max Sender (monophonic)** — `scamp:Playback/MIDI CC & arbitrary parameters/scamp_to_max_monophonic`
...
- **Max Sender (polyphonic)** — `scamp:Playback/MIDI CC & arbitrary parameters/scamp_to_max_polyphonic`
...
- **Save and Load a Performance** — `scamp:Playback/Other/save_and_load_performance`
...
- Computer Keyboard Input — *(home: `scamp:Tutorial/26_keyboard_input.py`)*
- Notating Computer Keyboard Input — *(home: `scamp:Notation & engraving/From live input/keyboard_input_with_notation.py`)*
- **Key Plane** *(scamp_extensions)* — `scamp_extensions:Interactivity & visualization/Keyboard input/key_plane_example.py`
...
- Mouse Input — *(home: `scamp:Tutorial/27_mouse_input.py`)*
- Indispensability *(scamp_extensions)* — *(home: `scamp_extensions:Composition & form/Algorithmic approaches/indispensibility_example.py`)*

### MIDI input

- Live MIDI input and output — *(home: `scamp:Tutorial/25_MIDI_in_out.py`)*
- **MIDI Keyboard Mapper** — `scamp:Interactivity & visualization/MIDI input/midi_keyboard_mapper.py`
...
- **Qt Interactive** — `scamp:Interactivity & visualization/Responding to a GUI/qt_interactive.py`
...
- **Conway's Game of Life (pygame version)** *(scamp_extensions)* — `scamp_extensions:Composition & form/Algorithmic approaches/conway.py`
  <br>Conway's Game of Life sonified, with a pygame window used for visualization instead of matplotlib. Original by Raphael Radna; visualization ported to pygame. — *API:* `Clock`, `Session`, `end`, `new_part`, `start_note` — *also under:* Interactivity & visualization › Visualization, Composition & form › Algorithmic approaches
- **L-System** *(scamp_extensions)* — `scamp_extensions:Composition & form/Algorithmic approaches/l_system_example.py`
...
- **Indispensability** *(scamp_extensions)* — `scamp_extensions:Composition & form/Algorithmic approaches/indispensibility_example.py`
  <br>Mouse position controls a rhythmic texture based on Barlow's beat indispensability (barlicity extension). — *API:* `Session`, `change_pitch`, `end`, `new_part`, `play_note`, `register_mouse_listener`, `start_note`, `wait` — *also under:* Interactivity & visualization › Mouse input, Composition & form › Algorithmic approaches

### Larger-scale form

- **Leaf Loops** *(scamp_extensions)* — `scamp_extensions:Composition & form/Larger-scale form/leaf_loops.py`
  <br>Generative process behind Marc Evanstein's "Leaf Loops" for violin and viola. The shapes, note attack points, and worm shape for several different leaves are found in the LeafPoints directory. — *API:* `Clock`, `Session`, `end_all_notes`, `get_time`, `new_midi_part`, `new_part`, `play_note`, `run_as_server`, `start_note` — *also under:* Interactivity & visualization › Visualization, Composition & form › Larger-scale form
- **Piano Phase** — `scamp:Composition & form/Larger-scale form/reich_piano_phase.py`
  <br>Steve Reich's Piano Phase: the same figure forked at 100 vs. 98 BPM, transcribed on one clock. — *API:* `QuantizationScheme`, `Session`, `fork`, `new_part`, `play_note`, `start_transcribing`, `to_score`, `wait` — *also under:* Time & clocks › Setting & changing tempo, Notation & engraving › Quantization, Composition & form › Larger-scale form
- **Barlicity** *(scamp_extensions)* — `scamp_extensions:Composition & form/Larger-scale form/evanstein_barlicity.py`
  <br>A large interactive piece built on harmonicity and indispensability (barlicity extension), with multidimensional-scaling visualization in Qt. This was the initial script for the piece, which ultimately became the notated work for piano and electronics that you can view here: https://www.youtube.com/watch?v=xMpET9KKOrw — *API:* `Envelope`, `Session`, `fork`, `get_beat`, `get_time`, `kill`, `new_part`, `play_chord`, `play_note`, `run_as_server`, `send_midi_cc`, `start_transcribing`, `...` — *also under:* Interactivity & visualization › Visualization, Composition & form › Larger-scale form
- **Evolving Form** *(scamp_extensions)* — `scamp_extensions:Composition & form/Larger-scale form/evolving_form`
  <br>A piece for cello, pianoteq (MIDI), and SuperCollider (OSC), shaped by envelope-driven formal parameters. See definitions.py and formal_parameters.py. — *API:* `Envelope`, `Moment`, `Session`, `fork`, `from_levels`, `kill`, `new_midi_part`, `new_osc_part`, `new_part`, `play_chord`, `play_note`, `set_rate_target`, `...` — *also under:* Time & clocks › Setting & changing tempo, Envelopes › As compositional parameters, Composition & form › Larger-scale form
- **TimeVaryingParameter Extension** *(scamp_extensions)* — `scamp_extensions:Composition & form/Larger-scale form/time_varying_parameter_example.py`
  <br>A script using the context-sensitive :class:`~expenvelope.envelope.Envelope` wrapper :class:`TimeVaryingParameter`, which reads into the underlying envelope at the current clock   's beat or time when called. — *API:* `Envelope`, `Session`, `current_clock`, `fork`, `from_levels`, `new_part`, `play_note`, `wait_for_children_to_finish` — *also under:* Envelopes › As compositional parameters, Composition & form › Larger-scale form
- **Lunar Trajectories** — `scamp:Composition & form/Larger-scale form/evanstein_lunar_trajectories.py`
