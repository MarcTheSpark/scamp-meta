...
- **Tempo Change** — `Tutorial/02_tempo_change.py`
  <br>Same as Hello World example, but at half-tempo. — *API:* `Session`, `new_part`, `play_note` — *also under:* Time & clocks › Setting & changing tempo
- **Blocking False** — `Tutorial/03_blocking_false.py`
  <br>Demonstrating the ability to have non-blocking calls to play_note. This plays two notes that each last for two beats, but overlapping by one beat. Passing `blocking=False` is essentially a single-note fork: playback continues on to the next line without waiting for the note to finish. — *API:* `Session`, `new_part`, `play_note`, `wait` — *also under:* Time & clocks › Forking & simultaneity
